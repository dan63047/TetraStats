...
class ClearData {
  ...
  final bool miniSpin;
  final bool spin;
  bool clearsGarbage = false;
  bool perfectClear = false;
  int id = -1;

  ClearData(this.title, this.lineclear, this.lines, this.miniSpin, this.spin, this.clearsGarbage);

  ClearData cloneWith(int i) {
    ClearData newOne = ClearData(title, lineclear, lines, miniSpin, spin, clearsGarbage)..id = i;
    return newOne;
  }

  bool get difficultClear {
    if (lines == 0) return false;
    if (lines >= 4 || miniSpin || spin)
      return true;
    else
      return false;
  }
  ...
  void toggleClearsGarbage() {
    clearsGarbage = !clearsGarbage;
  }
  ...
  int dealsDamage(int combo, int b2b, int previousB2B, Rules rules) {
    ...
    if (spin) {
      if (lines <= 5)
        ...
        damage += garbage[Lineclears.TSPIN_PENTA]! + 2 * (lines - 5);
    } else if (miniSpin) {
      damage += garbage[lineclear]!;
    } else {
      if (lines <= 5)
        ...
        damage += garbage[Lineclears.PENTA]! + (lines - 5);
    }

    if (difficultClear && b2b >= 1 && rules.b2b) {
      if (rules.b2bChaining)
        damage += BACKTOBACK_BONUS *
            ((1 + log(1 + (b2b) * BACKTOBACK_BONUS_LOG)).floor() +
                (b2b == 1 ? 0 : (1 + log(1 + (b2b) * BACKTOBACK_BONUS_LOG) % 1) / 3)); // but it should be b2b-1 ???
      else
        damage += 1; // if b2b chaining off
    }

    if (rules.combo && rules.comboTable != ComboTables.none) {
      if (combo >= 1) {
        if (rules.comboTable != ComboTables.multiplier)
          damage += combotable[rules.comboTable]![max(0, min(combo - 1, combotable[rules.comboTable]!.length - 1))];
        else
          damage *= (1 + COMBO_BONUS * (combo));
      }
      if (combo >= 2) {
...
Map<String, List<ClearData>> clearsExisting = {
  t.calcDestination.noSpinClears: [
    ClearData(t.calcDestination.noLineclear, Lineclears.ZERO, 0, false, false, false),
    ClearData(t.stats.lineClear.single, Lineclears.SINGLE, 1, false, false, false),
    ClearData(t.stats.lineClear.double, Lineclears.DOUBLE, 2, false, false, false),
    ClearData(t.stats.lineClear.triple, Lineclears.TRIPLE, 3, false, false, false),
    ClearData(t.stats.lineClear.quad, Lineclears.QUAD, 4, false, false, false)
  ],
  t.stats.spins: [
    ClearData("${t.stats.spin} ${t.stats.lineClears.zero}", Lineclears.TSPIN, 0, false, true, false),
    ClearData("${t.stats.spin} ${t.stats.lineClear.single}", Lineclears.TSPIN_SINGLE, 1, false, true, false),
    ClearData("${t.stats.spin} ${t.stats.lineClear.double}", Lineclears.TSPIN_DOUBLE, 2, false, true, false),
    ClearData("${t.stats.spin} ${t.stats.lineClear.triple}", Lineclears.TSPIN_TRIPLE, 3, false, true, false),
    ClearData("${t.stats.spin} ${t.stats.lineClear.quad}", Lineclears.TSPIN_QUAD, 4, false, true, false)
  ],
  "${t.stats.mini} ${t.stats.spins}": [
    ClearData("${t.stats.mini} ${t.stats.spin} ${t.stats.lineClears.zero}", Lineclears.TSPIN_MINI, 0, true, false, false),
    ClearData("${t.stats.mini} ${t.stats.spin} ${t.stats.lineClear.single}", Lineclears.TSPIN_MINI_SINGLE, 1, true, false, false),
    ClearData("${t.stats.mini} ${t.stats.spin} ${t.stats.lineClear.double}", Lineclears.TSPIN_MINI_DOUBLE, 2, true, false, false),
    ClearData("${t.stats.mini} ${t.stats.spin} ${t.stats.lineClear.triple}", Lineclears.TSPIN_MINI_TRIPLE, 3, true, false, false),
  ]
};
...
class Rules {
  ...
  bool surge = true;
  bool difficultClearWithGarbageBoost = true; // adds +1 line
...
class _DestinationCalculatorState extends State<DestinationCalculator> {
  ...
  Map<String, int> customClearsChoice = {t.calcDestination.noSpinClears: 5, t.stats.spins: 5};
  int idCounter = 0;
  Rules rules = Rules();
  ...
  Widget getCalculator() {
    return SingleChildScrollView(
      child: Column(
        children: [
          if (widget.constraints.maxWidth > 768.0)
            Card(
              child: Center(
                  child: Padding(
                  ...
                  Expanded(
                    ...
                    child: TextField(
                      ...
                      decoration: InputDecoration(
                          suffix: apmController.value.text.isNotEmpty ? Text("APM") : null,
                          alignLabelWithHint: true,
                          hintText: widget.constraints.maxWidth > 768.0
                              ? t.calcDestination.placeholders(stat: t.stats.apm.short)
                              : t.stats.apm.short),
                    ),
                  )),
                  Expanded(
                    ...
                    child: TextField(
                      ...
                      keyboardType: TextInputType.number,
                      decoration: InputDecoration(
                          suffix: ppsController.value.text.isNotEmpty ? Text("PPS") : null,
                          alignLabelWithHint: true,
                          hintText: widget.constraints.maxWidth > 768.0
                              ? t.calcDestination.placeholders(stat: t.stats.pps.short)
                              : t.stats.pps.short),
                    ),
                  )),
                  Expanded(
                    ...
                    child: TextField(
                      ...
                      keyboardType: TextInputType.number,
                      decoration: InputDecoration(
                          suffix: vsController.value.text.isNotEmpty ? Text("VS") : null,
                          alignLabelWithHint: true,
                          hintText: widget.constraints.maxWidth > 768.0
                              ? t.calcDestination.placeholders(stat: t.stats.vs.short)
                              : t.stats.vs.short),
                    ),
                  )),
                  TextButton(
          ...
          if (nerdStats != null) Card(child: NerdStatsThingy(nerdStats: nerdStats!, width: widget.constraints.minWidth)),
          if (playstyle != null) Card(child: Graphs(apm!, pps!, vs!, nerdStats!, playstyle!)),
  ...
  Widget rSideDamageCalculator(double width, bool hasSidebar) {
    return SizedBox(
      ...
      child: clears.isEmpty
          ? InfoThingy(t.calcDestination.damageCalcTip)
          : Card(
              child: Column(
                children: [
                  Expanded(
                      ...
                      Padding(
                        ...
                        child: Row(
                          children: [
                            ...
                            Spacer(),
                            Text(intf.format(totalDamage),
                                style: TextStyle(fontFamily: "Eurostile Round Extended", fontSize: 36, fontWeight: ui.FontWeight.w100))
                          ],
                        ),
                      ),
                      Row(
  ...
  Widget getDamageCalculator() {
    ...
    for (var key in clearsExisting.keys) {
      ...
      for (ClearData data in clearsExisting[key]!)
        rSideWidgets.add(Card(
          child: ListTile(
            title: Text(data.title),
            subtitle: Text("${data.dealsDamage(0, 0, 0, rules)} damage${data.difficultClear ? ", difficult" : ""}",
      ...
      if (key != "${t.stats.mini} ${t.stats.spins}")
        rSideWidgets.add(Card(
          child: ListTile(
    ...
    for (ClearData lineclear in clears) {
      previousB2B = b2b;
      if (lineclear.difficultClear)
        b2b++;
      else if (lineclear.lines > 0) b2b = -1;
      if (lineclear.lines > 0)
        combo++;
      else
        combo = -1;
      ...
      int surgeDmg = (!lineclear.difficultClear && rules.surge && previousB2B >= rules.surgeInitAtB2b && b2b == -1)
          ? rules.surgeInitAmount + (previousB2B - rules.surgeInitAtB2b)
          : 0;
      ...
      if (lineclear.clearsGarbage) {
        normalDmg += 1;
        dmg += 1;
      }
      int comboDmg = dmg - normalDmg - b2bDmg - surgeDmg - pcDmg;
      lSideWidgets.add(ListTile(
          ...
          children: [
            IconButton(
                onPressed: () {
                  setState(() {
                    clears.removeWhere(
                      (element) => element.id == lineclear.id,
                    );
                  });
                },
            ...
            if (lineclear.lines > 0)
              IconButton(
                  ...
                  icon: Icon(Icons.local_parking_outlined, color: lineclear.perfectClear ? Colors.white : Colors.grey.shade800)),
            if (lineclear.difficultClear)
              IconButton(
                  onPressed: () {
                    setState(() {
                      lineclear.toggleClearsGarbage();
                    });
                  },
                  icon: Icon(Icons.recycling, color: lineclear.clearsGarbage ? Colors.white : Colors.grey.shade800)),
          ],
        ),
        title: Text(
            ...
            children: [
              SizedBox(
                ...
                child: DefaultTabController(
                    length: widget.constraints.maxWidth > 768.0 ? 2 : 3,
                    child: Column(
                      mainAxisSize: MainAxisSize.min,
                      children: [
                        Card(
                                          ...
                                          trailing: SizedBox(
                                              ...
                                              child: TextField(
                                                ...
                                                onChanged: (value) => setState(() {
                                                  rules.multiplier = double.parse(value);
                                                }),
                                              )),
                                        ),
                                        ListTile(
                                          ...
                                          trailing: SizedBox(
                                              ...
                                              child: TextField(
                                                ...
                                                onChanged: (value) => setState(() {
                                                  rules.pcDamage = int.parse(value);
                                                }),
                                              )),
                                        ),
                                      ],
                                        ...
                                        if (rules.combo)
                                          ListTile(
                                            ...
                                            trailing: DropdownButton(
                                              items: [
                                                for (var v in ComboTables.values)
                                                  if (v != ComboTables.none)
                                                    DropdownMenuItem(value: v.index, child: Text(comboTablesNames[v]!))
                                              ],
                                              value: rules.comboTable.index,
                                              onChanged: (v) => setState(() {
                                                rules.comboTable = ComboTables.values[v!];
                                              }),
                                            ),
                                          )
                                      ],
                                          ...
                                          ListTile(
                                            ...
                                            trailing: SizedBox(
                                                ...
                                                child: TextField(
                                                  ...
                                                  onChanged: (value) => setState(() {
                                                    rules.surgeInitAtB2b = int.parse(value);
                                                  }),
                                                )),
                                          ),
                                        if (rules.surge)
                                          ListTile(
                                            ...
                                            trailing: SizedBox(
                                                ...
                                                child: TextField(
                                                  ...
                                                  onChanged: (value) => setState(() {
                                                    rules.surgeInitAmount = int.parse(value);
                                                  }),
                                                )),
                                          ),
                                      ],
    ...
    return Column(
      children: [
        SizedBox(
            ...
            child: switch (calcCard) { CalcCards.calc => getCalculator(), CalcCards.damage => getDamageCalculator() }),
        if (widget.constraints.maxWidth > 768.0)
